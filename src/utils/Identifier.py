...
class Identifier():
    ...
    def print_table(self, format = 'plain'):
        ...
        if format.lower() == 'latex':
            ...
            # Motor inertia
            if par_per_link == 11:
                params = np.hstack([self.robot.get_par_DYN().reshape(n,10), 
                                    self.robot.get_par_Ia().reshape(n,1)]).reshape(n, 11)
            else:
    ...
    def export(self, path = None):
        ...
        with open(config_path, "w") as f:
            f.write(f"# This file were autogenerated on {datetime.datetime.now()} from the parameters identification routine\n")
